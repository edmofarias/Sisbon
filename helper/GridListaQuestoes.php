 ...
 */
  $aColumns =
  array('id','nome','correta','pontos','materia');
  
  //{{{ Indexed column (used for fast and accurate table cardinality) 
  $sIndexColumn = "id";
  //}}}
  //{{{ DB table to use 
  $sTable = "cjaf_questoes";
  //}}}
 ...
 */
  ...
  {
  	$sql = 'SELECT nome FROM cjaf_materias WHERE id = "'.$id.'"';
...
//{{{ Output
  ...
  //print_r($aRow);
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
      ...
      else if ( $aColumns[$i] != ' ' )
      {
      	switch ($aColumns[$i]) {
      		...
      		case 'materia': $valor = '<center>'.getNomeMateria($aRow[$aColumns[$i]]).'</center>'; break;
    ...
    $iEditar = '<a title="Editar" href="formAluno.php?id='.$aRow[$aColumns[0]].'" >Editar</a>';
    $iRemover = '<a title="Excluir" id="excluir_'.$aRow[$aColumns[0]].'" href="#null" resposta="listaQuestoes.php" 
    			titulo="Excluir Registro" caixa="dialog_excluir" action="../controllers/questoes.php" 
    			onclick="excluirCadastro(\'excluir_'.$aRow[$aColumns[0]].'\',\''.$aRow[$aColumns[0]].'\')">Excluir</a>';
